...
void graphics_Image_draw(graphics_Image const* image, graphics_Quad const* quad,
  ...
  graphics_drawArray(quad, &tr2d, moduleData.imageVAO, moduleData.imageIBO, 0, 4, GL_TRIANGLE_STRIP, GL_UNSIGNED_BYTE, graphics_getColor(), image->width * quad->w, image->height * quad->h);
